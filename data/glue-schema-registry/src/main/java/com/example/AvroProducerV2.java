...
public class AvroProducerV2 {
    private static final Logger logger = LoggerFactory.getLogger(AvroProducerV2.class);
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Usage: AvroProducerV2 <properties-file> [bootstrap-servers-override]");
        ...
        // Create producer
        KafkaProducer<String, PaymentV2> producer = new KafkaProducer<>(props);

        try {
            // Send 10 messages
            for (int i = 1; i <= 10; i++) {
                // Create PaymentV2 object with optional description field
                PaymentV2 payment = PaymentV2.newBuilder()
                    .setPaymentId("payment-v2-" + i)
                    .setAmount(150.0 * i + (i * 0.5))
                    .setTimestamp(System.currentTimeMillis())
                    .setDescription("Payment for order #" + i)
                    .build();

                // Create producer record
                ProducerRecord<String, PaymentV2> record = new ProducerRecord<>(
                    TOPIC_NAME,
                ...
                logger.info("Sent payment V2: paymentId={}, amount={}, description={}, timestamp={}", 
                           payment.getPaymentId(), payment.getAmount(), payment.getDescription(), payment.getTimestamp());
            ...
            // Flush and close
            producer.flush();
            logger.info("Producer V2 completed. Sent 10 messages with schema version 2.");
